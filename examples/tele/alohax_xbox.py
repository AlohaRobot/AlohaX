#!/usr/bin/env python3
"""
AlohaX Xbox Controller Teleoperation - Direct Hardware Control
...
- Left Stick: 左臂控制 (左右=wrist_roll, 上下=elbow_flex, 按下+上下=wrist_flex)
- L/LB + Left Stick: 左臂肩部控制 (左右=shoulder_pan, 上下=shoulder_lift)
- D-pad Up/Down: 垂直升降 (+/-)
- D-pad Left/Right: 速度减/加 (对应Joycon的-=按键功能)
- LB: 左臂夹爪

- Right Stick: 右臂控制 (左右=wrist_roll, 上下=elbow_flex, 按下+上下=wrist_flex)
- R/RB + Right Stick: 右臂肩部控制 (左右=shoulder_pan, 上下=shoulder_lift)
...
LEFT_KEYMAP = {
    ...
    'shoulder_lift+': 'l_up', 'shoulder_lift-': 'l_down',  # 反向 id2
    'wrist_roll+': 'stick_right', 'wrist_roll-': 'stick_left',
    'elbow_flex+': 'stick_up', 'elbow_flex-': 'stick_down',  # 反向 id3
    'wrist_flex+': 'stick_pressed_up', 'wrist_flex-': 'stick_pressed_down',  # 反向 id4
    'gripper+': 'bumper',  # 现在用 bumper (LT/RT) 来控制夹爪！
    "vertical_lift+": 'dpad_up', "vertical_lift-": 'dpad_down',
}
...
RIGHT_KEYMAP = {
    ...
    'shoulder_lift+': 'r_up', 'shoulder_lift-': 'r_down',  # 反向 id2
    'wrist_roll+': 'stick_right', 'wrist_roll-': 'stick_left',
    'elbow_flex+': 'stick_up', 'elbow_flex-': 'stick_down',  # 反向 id3
    'wrist_flex+': 'stick_pressed_up', 'wrist_flex-': 'stick_pressed_down',  # 反向 id4
    'gripper+': 'bumper',  # 同理，用 bumper (LT/RT) 来控制夹爪！
}
...
class TeleopDisplay:
    ...
    def init_curses(self):
        ...
        self.stdscr.keypad(True)  # 启用键盘模式以支持方向键等特殊按键
        curses.curs_set(0)
        curses.start_color()
    ...
    def display(self):
        ...
        try:
            ...
            line += 1
            title_text = "AlohaX Xbox Teleop - Display OK!"
            ...
            if line < height:
                safe_addstr(line, 0, f"[ CONTROLS ] | LStick=LeftArm | RStick=RightArm | D-pad=Lift/Speed | YAXB=Move | Start=Torque | Back=Reset | Torque: {self.torque_status}", BRIGHT_WHITE)
...
class SimpleTeleopArm:
    ...
    def handle_keys(self, key_state):
        if key_state.get('gripper+'):
            self.target_positions["gripper"] = 1.2  # 按下时张开！
        else:
            self.target_positions["gripper"] = 0.0  # 默认闭合！
...
# Xbox控制器对象
_xbox_controller = None
_controller_initialized = False
_controller_error_shown = False
...
def init_xbox_controller():
    """初始化Xbox控制器"""
    global _xbox_controller, _controller_initialized, _controller_error_shown
    try:
        if pygame.joystick.get_count() > 0:
            _xbox_controller = pygame.joystick.Joystick(0)
            _xbox_controller.init()
            _controller_initialized = True
            _controller_error_shown = False
            print(f"[OK] Xbox Controller connected: {_xbox_controller.get_name()}")
            return True
        else:
            if not _controller_error_shown:
                print("[WARNING] No Xbox controller detected. Please connect an Xbox controller.")
                _controller_error_shown = True
            return False
    except Exception as e:
        if not _controller_error_shown:
            print(f"[ERROR] Failed to initialize Xbox controller: {e}")
            _controller_error_shown = True
        return False

# ================== Xbox控制器状态获取函数 ==================
_last_debug_time = 0
def get_xbox_controller_state():
    """获取Xbox控制器状态"""
    global _xbox_controller, _controller_initialized, _last_debug_time
    
    # 如果控制器未初始化，尝试初始化
    if not _controller_initialized:
        init_xbox_controller()
    
    state = {
        # 底盘控制 (Xbox: Y=前进, A=后退, X=左转, B=右转)
        'base_forward': False,
        'base_backward': False,
        'base_left': False,
        'base_right': False,
        
        # 升降控制 (Xbox: D-pad Up/Down)
        'vertical_lift+': False,
        'vertical_lift-': False,
        
        # 速度控制 (Xbox: D-pad Left/Right = speed-/speed+)
        'speed+': False,
        'speed-': False,
        
        # 特殊功能 (Xbox: Back=复位, Start=扭矩切换)
        'reset': False,
        'torque_toggle': False,
        ...
        'left_shoulder': False,  # LB 用于肩部控制
        'left_bumper': False,    # LT 用于夹爪
        ...
        'right_shoulder': False,  # RB 用于肩部控制
        'right_bumper': False,    # RT 用于夹爪
    }
    
    if not _controller_initialized or _xbox_controller is None:
        return state
    ...
    try:
        # 获取控制器信息
        num_buttons = _xbox_controller.get_numbuttons()
        num_axes = _xbox_controller.get_numaxes()
        num_hats = _xbox_controller.get_numhats()
        
        # 读取所有按钮状态用于调试
        button_states = []
        for i in range(num_buttons):
            if _xbox_controller.get_button(i):
                button_states.append(f"B{i}")
        
        # 读取所有摇杆状态用于调试
        axis_states = []
        for i in range(num_axes):
            val = _xbox_controller.get_axis(i)
            if abs(val) > 0.1:
                axis_states.append(f"A{i}:{val:.2f}")
        
        # 读取所有hat状态用于调试
        hat_states = []
        for i in range(num_hats):
            val = _xbox_controller.get_hat(i)
            if val != (0, 0):
                hat_states.append(f"H{i}:{val}")
        
        # ---------------------- 底盘控制: Y/A/X/B ----------------------
        if num_buttons > 3:
            if _xbox_controller.get_button(3):  # Y
                state['base_forward'] = True
            if _xbox_controller.get_button(0):  # A
                state['base_backward'] = True
            if _xbox_controller.get_button(2):  # X
                state['base_left'] = True
            if _xbox_controller.get_button(1):  # B
                state['base_right'] = True
        
        # ---------------------- 扳机键/夹爪: LT/RT (通过轴输入) ----------------------
        # 从调试信息看，这个控制器用 A2/A5
        # 初始状态是 -1.0，按下时变成 1.0，所以只检测 > 0 才算按下
        lt = _xbox_controller.get_axis(2)
        rt = _xbox_controller.get_axis(5)
        lt_pressed = False
        rt_pressed = False
        if lt > 0.0:
            state['left_bumper'] = True
            lt_pressed = True
        if rt > 0.0:
        ...
        # ---------------------- 肩键/肩部控制: LB/RB ----------------------
        if num_buttons > 5:
            if _xbox_controller.get_button(4):  # LB
                state['left_shoulder'] = True
            if _xbox_controller.get_button(5):  # RB
                state['right_shoulder'] = True
        
        # ---------------------- 特殊功能: Back/Start ----------------------
        if num_buttons > 9:
            if _xbox_controller.get_button(6):  # Back
                state['reset'] = True
            if _xbox_controller.get_button(7):  # Start
                state['torque_toggle'] = True
        
        # ---------------------- 摇杆按下 (从调试信息得到) ----------------------
        if num_buttons > 10:
            if _xbox_controller.get_button(9):  # Left Stick Press (B9)
                state['left_stick_pressed'] = True
            if _xbox_controller.get_button(10):  # Right Stick Press (B10)
                state['right_stick_pressed'] = True
        
        # ---------------------- 左摇杆 (A0=x, A1=y) ----------------------
        if num_axes > 1:
            left_x = _xbox_controller.get_axis(0)
            left_y = _xbox_controller.get_axis(1)
            if left_y < -STICK_DEADZONE:
                state['left_stick_up'] = True
            if left_y > STICK_DEADZONE:
                state['left_stick_down'] = True
            if left_x < -STICK_DEADZONE:
                state['left_stick_left'] = True
            if left_x > STICK_DEADZONE:
                state['left_stick_right'] = True
        
        # ---------------------- 右摇杆 (A3=x, A4=y) ----------------------
        if num_axes > 4:
            right_x = _xbox_controller.get_axis(3)
            right_y = _xbox_controller.get_axis(4)
            if right_y < -STICK_DEADZONE:
                state['right_stick_up'] = True
            if right_y > STICK_DEADZONE:
                state['right_stick_down'] = True
            if right_x < -STICK_DEADZONE:
                state['right_stick_left'] = True
            if right_x > STICK_DEADZONE:
                state['right_stick_right'] = True
        
        # ---------------------- D-pad (十字按键) ----------------------
        if num_hats > 0:
            hat = _xbox_controller.get_hat(0)  # (x, y)
            if hat[1] == 1:  # D-pad Up
                state['vertical_lift+'] = True
            if hat[1] == -1:  # D-pad Down
                state['vertical_lift-'] = True
            if hat[0] == -1:  # D-pad Left
                state['speed-'] = True
            if hat[0] == 1:  # D-pad Right
                state['speed+'] = True
        elif num_axes > 5:
            hat_x = _xbox_controller.get_axis(4)
            hat_y = _xbox_controller.get_axis(5)
            if hat_y < -0.5:
                state['vertical_lift+'] = True
            if hat_y > 0.5:
                state['vertical_lift-'] = True
            ...
            if _xbox_controller.get_button(11):
                state['vertical_lift+'] = True
            if _xbox_controller.get_button(12):
                state['vertical_lift-'] = True
            if _xbox_controller.get_button(13):
                state['speed-'] = True
            if _xbox_controller.get_button(14):
                state['speed+'] = True
        
        # ---------------------- 调试输出 (每2秒一次) ----------------------
        current_time = time.time()
        if current_time - _last_debug_time > 2.0:
            _last_debug_time = current_time
            # 打印所有按钮的状态（不只是按下的）
            all_btn = []
            for i in range(num_buttons):
                val = _xbox_controller.get_button(i)
                all_btn.append(f"B{i}:{val}")
            # 打印所有轴的值
            all_axis = []
            for i in range(num_axes):
                val = _xbox_controller.get_axis(i)
                if abs(val) > 0.1:
                    all_axis.append(f"A{i}:{val:.2f}")
                else:
                    all_axis.append(f"A{i}:0")
            
            active_keys = [k for k, v in state.items() if v]
            debug_msg = f"[DEBUG] All BTN: {', '.join(all_btn)} | All AXIS: {', '.join(all_axis)}"
            if active_keys:
                debug_msg += f" | ACTIVE: {active_keys}"
            print(debug_msg)
            
    except Exception as e:
        print(f"[ERROR] Controller read: {e}")
    
    return state

# ================== Xbox风格按键状态转换函数 ==================
_last_keystate_debug_time = 0
def get_xbox_key_state(key_state, keymap, is_left=True):
    """将键盘按键状态转换为Xbox控制器风格的状态"""
    global _last_keystate_debug_time
    
    state = {}
    
    stick_up = key_state.get('left_stick_up' if is_left else 'right_stick_up', False)
    stick_down = key_state.get('left_stick_down' if is_left else 'right_stick_down', False)
    stick_left = key_state.get('left_stick_left' if is_left else 'right_stick_left', False)
    stick_right = key_state.get('left_stick_right' if is_left else 'right_stick_right', False)
    stick_pressed = key_state.get('left_stick_pressed' if is_left else 'right_stick_pressed', False)
    shoulder_button = key_state.get('left_shoulder' if is_left else 'right_shoulder', False)
    bumper_button = key_state.get('left_bumper' if is_left else 'right_bumper', False)
    # 注意：只有 shoulder_button (LB/RB) 才用于复合按键！
    # bumper_button (LT/RT) 只用于夹爪，不参与其他复合控制
    shoulder_active = shoulder_button
    
    for action, control in keymap.items():
        if control == 'stick_up':
            state[action] = (not stick_pressed) and (not shoulder_active) and stick_up
        elif control == 'stick_down':
            state[action] = (not stick_pressed) and (not shoulder_active) and stick_down
        elif control == 'stick_left':
            state[action] = (not stick_pressed) and (not shoulder_active) and stick_left
        elif control == 'stick_right':
            state[action] = (not stick_pressed) and (not shoulder_active) and stick_right
        elif control == 'stick_pressed_up':
            state[action] = stick_pressed and (not shoulder_active) and stick_up
        elif control == 'stick_pressed_down':
            state[action] = stick_pressed and (not shoulder_active) and stick_down
        elif control == 'stick_pressed_left':
            state[action] = stick_pressed and (not shoulder_active) and stick_left
        elif control == 'stick_pressed_right':
            state[action] = stick_pressed and (not shoulder_active) and stick_right
        elif control == 'l_up' or control == 'r_up':
            state[action] = shoulder_active and stick_up
        elif control == 'l_down' or control == 'r_down':
            state[action] = shoulder_active and stick_down
        elif control == 'l_left' or control == 'r_left':
            state[action] = shoulder_active and stick_left
        elif control == 'l_right' or control == 'r_right':
            state[action] = shoulder_active and stick_right
        elif control == 'bumper':
            # 专门用于夹爪：左臂用 bumper_button (LT)，右臂用 bumper_button (RT)
            state[action] = bumper_button
        elif control == 'lb':
            # 保持兼容性：如果有旧的 'lb' 项，只用于左臂的 shoulder_button (LB)
            state[action] = shoulder_button if is_left else False
        elif control == 'rb':
            # 保持兼容性：如果有旧的 'rb' 项，只用于右臂的 shoulder_button (RB)
            state[action] = shoulder_button if not is_left else False
        elif control == 'dpad_up':
            state[action] = key_state.get('vertical_lift+', False) if is_left else False
    ...
    # 调试输出 (每2秒一次)
    current_time = time.time()
    if current_time - _last_keystate_debug_time > 2.0:
        _last_keystate_debug_time = current_time
        active_actions = [k for k, v in state.items() if v]
        if active_actions:
            arm_name = "LEFT" if is_left else "RIGHT"
            print(f"[DEBUG] {arm_name} ARM: ACTIVE={active_actions}")
...
class AlohaXTeleopNode(Node):
    def __init__(self, port1=DEFAULT_PORT1, port2=DEFAULT_PORT2, display=None):
        super().__init__('alohax_teleop_xbox')
    ...
    def timer_callback(self):
        try:
            ...
            key_state = get_xbox_controller_state()
            
            start_pressed = key_state.get('torque_toggle', False)
            back_pressed = key_state.get('reset', False)

            if start_pressed and not self.last_home_press:
                ...
                try:
                    ...
                    pass
            elif back_pressed and not self.last_home_press:
                ...
                if self.display:
                    self.display.set_reset_flag(False)
            self.last_home_press = start_pressed or back_pressed
            
            plus_pressed = key_state.get('speed+', False)
            minus_pressed = key_state.get('speed-', False)

            if plus_pressed and not self.last_plus_press and not self.is_homing:
            ...
            if minus_pressed and not self.last_minus_press and not self.is_homing:
            ...
            left_key_state = get_xbox_key_state(key_state, LEFT_KEYMAP, is_left=True)
            ...
            right_key_state = get_xbox_key_state(key_state, RIGHT_KEYMAP, is_left=False)
...
# ================== 主函数 ==================
def parse_args():
    parser = argparse.ArgumentParser(description="AlohaX Xbox Teleoperation - Direct Hardware Control")
