...
from ...utils.encoding_utils import decode_sign_magnitude, encode_sign_magnitude
...
class FeetechMotorsBus(MotorsBus):
    ...
    def _connect(self, handshake: bool = True) -> None:
        """Override to set baud rate before handshake."""
        try:
            if not self.port_handler.openPort():
                raise OSError(f"Failed to open port '{self.port}'.")
            # Set the baud rate to the default before performing handshake
            self.port_handler.setBaudRate(self.default_baudrate)
            logger.debug(f"Set baud rate to {self.default_baudrate} after opening port.")
            if handshake:
                self._handshake()
    ...
    def configure_motors(self, return_delay_time=0, maximum_acceleration=254, acceleration=254) -> None:
        for motor in self.motors:
            ...
            # 'Return_Delay_Time' address). We ensure this is reduced to the minimum of 2µs (value of 0).
            self.write("Return_Delay_Time", motor, return_delay_time)
            # Set 'Maximum_Acceleration' to 254 to speedup acceleration and deceleration of motors.
            if self.protocol_version == 0:
    ...
    def _read_firmware_version(self, motor_ids: list[int], raise_on_error: bool = True) -> dict[int, tuple[int, int]]:
        firmware_versions = {}
        for motor_id in motor_ids:
            error, major = self._read_single_field(motor_id, "Firmware_Major_Version")
            if error and raise_on_error:
                raise ConnectionError(
                    f"Failed to read Firmware_Major_Version from motor {motor_id}: {error}"
                )
            error, minor = self._read_single_field(motor_id, "Firmware_Minor_Version")
            if error and raise_on_error:
                raise ConnectionError(
                    f"Failed to read Firmware_Minor_Version from motor {motor_id}: {error}"
                )
            firmware_versions[motor_id] = (major, minor)
        return firmware_versions

    def _read_single_field(self, motor_id: int, field: str) -> tuple[str | None, int]:
        import scservo_sdk as scs

        model = self._id_to_model(motor_id)
        addr, length = get_address(self.model_ctrl_table, model, field)
        if length == 1:
            comm, error, value = self.packet_handler.read1ByteTxRx(self.port_handler, motor_id, addr)
        elif length == 2:
            comm, error, value = self.packet_handler.read2ByteTxRx(self.port_handler, motor_id, addr)
        elif length == 4:
            comm, error, value = self.packet_handler.read4ByteTxRx(self.port_handler, motor_id, addr)
        else:
            raise ValueError(f"Unsupported length {length} for field {field}")

        if comm != scs.COMM_SUCCESS:
            return self.packet_handler.getTxRxResult(comm), 0
        if error != 0:
            return self.packet_handler.getRxPacketError(error), 0
        return None, value
    ...
    def broadcast_ping(self, num_retry: int = 0, raise_on_error: bool = False) -> dict[int, int] | None:
        ...
        Args:
            ...
            raise_on_error (bool, optional): When `True` failures raise an exception instead of
